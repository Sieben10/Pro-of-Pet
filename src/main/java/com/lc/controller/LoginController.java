...
import com.lc.utils.AliDayunSms;
...
public class LoginController {
	...
	@ResponseBody
	public Result<String> register(HttpServletResponse response, RegisterVo registerVo,String code,HttpSession session ,String useryouxiang) {
		String code1 = (String) session.getAttribute("duanxincode");

		if(!code.equals(code1)){
			Result<String> rst = Result.success("error");
			rst.setMsg("验证码错误");
			rst.setCode(2);
			return rst;
		}
		...
		String result = userService.register(response, registerVo, useryouxiang);
	...
	@ResponseBody
	public Result<String> sendSMS(HttpServletResponse response, String phone,HttpSession session,Integer type) throws JSONException {
		if(type==1){
			User u = userService.getByPhone(phone);
			if(u!=null){
				Result<String> rst = Result.success("");
				rst.setCode(0);
				rst.setMsg("手机号已存在，请更换手机号注册");
				return rst;
			}
		}
		if(type==2){
		...
		try{
			String code = AliDayunSms.sendPhone(phone);
			session.setAttribute("duanxincode",code);
		}catch (Exception e){
